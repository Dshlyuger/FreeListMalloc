...
static inline void* next(void* bp) {
    unsigned int offsetOfNext = GET(bp);
    void * nextBlock = (void *)((offsetOfNext) + heap_listp);
    return nextBlock;
}

static inline void* prev(void* bp) {
    char* startOfPrev = (((char *)(bp)) + WSIZE);
    unsigned int offsetOfPrev = GET(startOfPrev);
...
static void addToFront(char* bp){
    ...
    if(headOfFreeList == heap_listp) {
        ...
        int offsetOfCurrentHead = headOfFreeList - heap_listp;
        PUT(nextWord,offsetOfCurrentHead);
        
        // Make old head previous point to new head with offset;
        
        int offsetOfNewBlock = bp - heap_listp;
        // This line is causing problems, might want to find out why.
        PUT(headOfFreeList,offsetOfNewBlock);
    }
...
static void printblock(void *bp)
{
    ...
    printf("Header Size %zu, Header Allocated %zu, Footer Size %zu, Footer Allocated %zu", hsize, halloc, fsize, falloc);
